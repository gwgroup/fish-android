package com.ypcxpt.fish.main.contract;

import com.ypcxpt.fish.library.presenter.IPresenter;
import com.ypcxpt.fish.library.view.IView;
import com.ypcxpt.fish.main.model.CollectionInfo;

public interface CollectionContract {
    interface View extends IView {
        void showCollections(CollectionInfo collectionInfo);
    }

    interface Presenter extends IPresenter {
        //跳转收藏详情进行操作
        void skipCollectionDetail(CollectionInfo.RowsBean rowsBean);
        void getCollections(String pageIndex, String pageNum);
    }
}
