...
public class FloatMenu extends PopupWindow {
	...
	private void generateLayout(int itemWidth) {
		...
		menuLayout.setBackgroundDrawable(ContextCompat.getDrawable(context, com.noober.menu.R.drawable.bg_shadow));
		menuLayout.setOrientation(LinearLayout.VERTICAL);
		int padding = Display.dip2px(context, 15);
		for(int i = 0; i < menuItemList.size(); i ++){
			TextView textView = new TextView(context);
			textView.setClickable(true);
			textView.setBackgroundDrawable(ContextCompat.getDrawable(context, com.noober.menu.R.drawable.selector_item));
			textView.setPadding(padding, padding, padding, padding);
			textView.setWidth(itemWidth);
			textView.setGravity(Gravity.CENTER_VERTICAL | Gravity.START);
			textView.setTextSize(17);
			textView.setTextColor(App.getInstance().getResources().getColor(R.color.common_2f2f2f));
	...
	private void readItem(AttributeSet attrs) {
		TypedArray a = context.obtainStyledAttributes(attrs, com.noober.menu.R.styleable.MenuItem);
		CharSequence itemTitle = a.getText(com.noober.menu.R.styleable.MenuItem_menu_title);
		int itemIconResId = a.getResourceId(com.noober.menu.R.styleable.MenuItem_icon, View.NO_ID);
